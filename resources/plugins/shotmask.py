...
class HShotMaskLocator(omui.MPxLocatorNode):
    """
    """

    NAME = "hshotmask"
    TYPE_ID = om.MTypeId(0x0011A885)
    DRAW_DB_CLASSIFICATION = "drawdb/geometry/hshotmask"
    DRAW_REGISTRANT_ID = "HShotMaskNode"
    ...
    def __init__(self):
        """
        """
        super(HShotMaskLocator, self).__init__()
    ...
    def initialize(cls):
        ...
        HShotMaskLocator.addAttribute(camera_name)
        ...
        attr.setMin(0)
        attr.setMax(6)
        HShotMaskLocator.addAttribute(counter_position)
        ...
        attr.setMin(1)
        attr.setMax(6)
        HShotMaskLocator.addAttribute(counter_padding)
        ...
        for i in range(0, len(cls.TEXT_ATTRS), 2):
            ...
            t_attr.storable = True
            t_attr.keyable = True
            HShotMaskLocator.addAttribute(position)
        ...
        t_attr.storable = True
        t_attr.keyable = True
        HShotMaskLocator.addAttribute(font_name)
        ...
        attr.storable = True
        attr.keyable = True
        HShotMaskLocator.addAttribute(font_color)
        ...
        attr.setMin(0.0)
        attr.setMax(1.0)
        HShotMaskLocator.addAttribute(font_alpha)
        ...
        attr.setMin(0.1)
        attr.setMax(2.0)
        HShotMaskLocator.addAttribute(font_scale)
        ...
        attr.storable = True
        attr.keyable = True
        HShotMaskLocator.addAttribute(top_border)
        ...
        attr.storable = True
        attr.keyable = True
        HShotMaskLocator.addAttribute(bottom_border)
        ...
        attr.setMin(0.0)
        attr.setMax(1.0)
        HShotMaskLocator.addAttribute(border_alpha)
        ...
        attr.setMin(0.5)
        attr.setMax(2.0)
        HShotMaskLocator.addAttribute(border_scale)


class HShotMaskData(om.MUserData):
    """
    """
    ...
    def __init__(self):
        """
        """
        super(HShotMaskData, self).__init__(False)  # don't delete after draw


class HShotMaskDrawOverride(omr.MPxDrawOverride):
    """
    """

    NAME = "Hshotmask_draw_override"

    def __init__(self, obj):
        """
        """
        super(HShotMaskDrawOverride, self).__init__(obj, HShotMaskDrawOverride.draw)
    ...
    def prepareForDraw(self, obj_path, camera_path, frame_context, old_data):
        ...
        """
        data = old_data
        if not isinstance(data, HShotMaskData):
            data = HShotMaskData()
        ...
        data.text_fields = []
        for i in range(0, len(HShotMaskLocator.TEXT_ATTRS), 2):
            data.text_fields.append(fnDagNode.findPlug(HShotMaskLocator.TEXT_ATTRS[i], False).asString())
        ...
        if counter_position > 0 and counter_position <= len(HShotMaskLocator.TEXT_ATTRS) / 2:
    ...
    def addUIDrawables(self, obj_path, draw_manager, frame_context, data):
        ...
        if camera.filmFit == om.MFnCamera.kHorizontalFilmFit:
            ...
            om.MGlobal.displayError("[hShotMask] Unknown Film Fit value")
            return
    ...
    def creator(obj):
        """
        """
        return HShotMaskDrawOverride(obj)
...
def initializePlugin(obj):
    ...
    try:
        pluginFn.registerNode(HShotMaskLocator.NAME,
                              HShotMaskLocator.TYPE_ID,
                              HShotMaskLocator.creator,
                              HShotMaskLocator.initialize,
                              om.MPxNode.kLocatorNode,
                              HShotMaskLocator.DRAW_DB_CLASSIFICATION)
    except:
        om.MGlobal.displayError("Failed to register node: {0}".format(HShotMaskLocator.NAME))

    try:
        omr.MDrawRegistry.registerDrawOverrideCreator(HShotMaskLocator.DRAW_DB_CLASSIFICATION,
                                                      HShotMaskLocator.DRAW_REGISTRANT_ID,
                                                      HShotMaskDrawOverride.creator)
    except:
        om.MGlobal.displayError("Failed to register draw override: {0}".format(HShotMaskDrawOverride.NAME))
...
def uninitializePlugin(obj):
    ...
    try:
        omr.MDrawRegistry.deregisterDrawOverrideCreator(HShotMaskLocator.DRAW_DB_CLASSIFICATION, HShotMaskLocator.DRAW_REGISTRANT_ID)
    except:
        om.MGlobal.displayError("Failed to deregister draw override: {0}".format(HShotMaskDrawOverride.NAME))

    try:
        pluginFn.deregisterNode(HShotMaskLocator.TYPE_ID)
    except:
        om.MGlobal.displayError("Failed to unregister node: {0}".format(HShotMaskLocator.NAME))
...
if __name__ == "__main__":
    ...
    plugin_name = "hshotmask.py"
    cmds.evalDeferred('if cmds.pluginInfo("{0}", q=True, loaded=True): cmds.unloadPlugin("{0}")'.format(plugin_name))
    cmds.evalDeferred('if not cmds.pluginInfo("{0}", q=True, loaded=True): cmds.loadPlugin("{0}")'.format(plugin_name))

    cmds.evalDeferred('cmds.createNode("hshotmask")')
